  ...
  return (
    <>
      <div
        data-show={show}
        onClick={onClose}
        className="fixed inset-0 z-[999] bg-black/40 opacity-0 transition-opacity duration-300 ease-in-out data-[show=true]:opacity-100"
      />
      <aside
        data-show={show}
        className="fixed top-0 left-0 z-[1000] flex h-screen w-[280px] -translate-x-full flex-col overflow-hidden rounded-tr-xl rounded-br-xl bg-[#F7F7F7] shadow-[2px_0_12px_rgba(0,0,0,0.2)] transition-transform duration-[400ms] ease-[cubic-bezier(0.2,0.8,0.2,1)] data-[show=true]:translate-x-0"
      >
        <header className="flex h-14 items-center justify-between border-b border-black/40 bg-[#f5f5f5] px-5">
          <h2 className="m-0 text-[18px] font-medium text-brand-primary">
            訂單管理平台
          </h2>
          <button
            type="button"
            onClick={onClose}
            className="cursor-pointer border-none bg-transparent text-2xl text-[#999] transition-colors hover:text-[#333]"
            aria-label="關閉側邊欄"
          >
